...
menuItems.forEach(item => {
    item.addEventListener("mouseenter", function() {
        ...
        // Ajout des images avec texte et bouton
        ...
        // Cache le menu en cliquant ailleurs
        document.addEventListener("click", function (event) {
        ...
        // Disparition du menu si la souris quitte la zone
menuContent.addEventListener("mouseleave", function() {
    timeout = setTimeout(() => {
        menuContent.style.display = "none";
    }, 300); // Délai avant disparition (300ms)
});
        
        // Sélectionne toutes les images à prévisualiser
    document.querySelectorAll(".preview-img").forEach(img => {
        img.addEventListener("mouseover", function() {
            this.dataset.original = this.src; // Sauvegarde l'image d'origine
            this.src = this.getAttribute("data-hover"); // Change l'image au survol
        });
    
        img.addEventListener("mouseout", function() {
            this.src = this.dataset.original; // Remet l'image d'origine
        });
    });
...
document.addEventListener("DOMContentLoaded", function () {
    let text = "MR THE AVEDJI";
    let index = 0;
    let speed = 150; // Vitesse d’écriture

    function typeWriter() {
        if (index < text.length) {
            document.getElementById("site-title").innerHTML += text.charAt(index);
            index++;
